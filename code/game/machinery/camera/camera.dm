		...
		for(var/mob/O in GLOB.player_list)
			if(istype(O, /mob/living/silicon/ai))
				...
				if(U.name == "Unknown")
					...
					to_chat(AI, "<b><a href='?src=[AI.UID()];track=[rhtml_encode(U.name)]'>[U]</a></b> holds <a href='?_src_=usr;show_paper=1;'>\a [itemname]</a> up to one of your cameras ...")
...
/obj/machinery/camera/proc/nano_structure()
	...
	cam["name"] = sanitize_russian(c_tag)
	cam["deact"] = !can_use()
	cam["camera"] = "\ref[src]"
	if(T)
		cam["x"] = T.x
