...
/obj/machinery/computer/library/checkout/Topic(href, href_list)
	...
	if(href_list["editbook"])
		buffer_book = copytext(sanitize_russian(input("Enter the book's title:") as text|null),1,MAX_MESSAGE_LEN)
	if(href_list["editmob"])
		buffer_mob = copytext(sanitize_russian(input("Enter the recipient's name:") as text|null),1,MAX_NAME_LEN)
	if(href_list["checkout"])
		var/datum/borrowbook/b = new /datum/borrowbook
		b.bookname = sanitize_russian(buffer_book)
		b.mobname = sanitize_russian(buffer_mob)
	...
	if(href_list["uploadauthor"])
		var/newauthor = copytext(sanitize_russian(input("Enter the author's name: ") as text|null),1,MAX_MESSAGE_LEN)
